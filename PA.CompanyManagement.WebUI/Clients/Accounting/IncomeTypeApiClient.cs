...
{
    public interface IIncomeTypeApiClient : IIncomeTypeRepository
    {

    }

    public class IncomeTypeApiClient : IIncomeTypeApiClient
    {
        private readonly HttpClient _client;

        public IncomeTypeApiClient(HttpClient client)
        {
            _client = client;
        }
        ...
        public async Task<IncomeTypeResponse> CreateAsync(IncomeTypeCreateRequest request)
        {
            try
            {
                ...
                var model = JsonConvert.DeserializeObject<IncomeTypeResponse>(data);
                return model!;
            }
            catch (Exception ex)
            {
                return new IncomeTypeResponse();
            }
        }
        ...
        public async Task<List<IncomeTypeResponse>> GetAllAsync()
        {
            try
            {
                ...
                var model = JsonConvert.DeserializeObject<List<IncomeTypeResponse>>(content);
                return model!;
            }
            catch (Exception ex)
            {
                return new List<IncomeTypeResponse>();
            }
        }
        ...
        public async Task<IncomeTypeResponse?> GetAsync(Guid id)
        {
            try
            {
                ...
                response.EnsureSuccessStatusCode();
                var content = await response.Content.ReadAsStringAsync();
                var model = JsonConvert.DeserializeObject<IncomeTypeResponse>(content);
                return model;
            }
            catch (Exception)
            {
        ...
        public async Task<DetailedIncomeTypeResponse?> GetDetailedAsync(Guid id)
        {
            try
            {
                ...
                response.EnsureSuccessStatusCode();
                var content = await response.Content.ReadAsStringAsync();
                var model = JsonConvert.DeserializeObject<DetailedIncomeTypeResponse>(content);
                return model;
            }
            catch (Exception)
            {
